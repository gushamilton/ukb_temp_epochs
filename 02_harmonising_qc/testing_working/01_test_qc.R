...
# Use the path to your parquet file
...
unique(d$id)
# --- 3. Process and Smooth ALL Participants ---
# This part now runs the full cleaning pipeline on the entire dataset.
...
# This model is now fit ONCE to the much smaller, downsampled dataset.
cat("Fitting global mixed-effects model (lmer)... This will be much faster now.\n")
...
# For each participant, compute the amplitude and frequency of the first peak in the Fourier amplitude spectrum,
# then plot the distributions of both across all participants
...
ggplot() +
  ...
  sampled_ids <- sample(unique(processed_data$id), 100)
