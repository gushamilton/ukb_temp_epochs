...
#
# v2 Changes:
# - FIXED: Corrected a critical bug in `preprocess_and_downsample` where calculating
#          residuals did not correctly handle missing values, leading to NaN results.
#          The fix uses `na.action = na.exclude` in the linear model.
...
# In Conda: conda install -c conda-forge r-base r-arrow r-dplyr r-data.table r-tidyr r-purrr r-lubridate r-zoo r-rcpproll
library(arrow)
library(dplyr)
library(data.table)
library(tidyr)
    ...
    mutate(
      temp_residuals_from_activity_30s = {
        if (sum(!is.na(temp) & !is.na(enmoTrunc)) >= 10 && var(enmoTrunc, na.rm = TRUE) > 1e-9) {
          # CRITICAL FIX: Using na.action = na.exclude ensures the output of residuals()
          # has the same length as the input data frame, with NAs inserted for rows
          # that were dropped. This makes it safe to use inside mutate().
          model <- lm(temp ~ enmoTrunc, data = cur_data(), na.action = na.exclude)
          residuals(model)
        } else {
          NA_real_ # This will be broadcast to the correct length by mutate
        }
      }
    ) %>%
    ungroup() %>%
...
calculate_fft_params <- function(df, temp_col_name) {
  ...
  # Perform FFT
  fft_result <- fft(temp_series - mean(temp_series, na.rm=TRUE))
...
stability_analysis <- model_results %>%
  ...
  summarise(
    ...
    cv_mesor_std = sd_mesor_std / abs(mean_mesor_std),
